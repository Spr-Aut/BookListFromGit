...
import com.example.test0_debug.tools.httpHelper.xmlParse.WordSenDataSet;
...
public class ReviewFragment extends Fragment implements View.OnClickListener {
    ...
    private void initData(boolean replaced) {
        ...
        if (cursor.moveToFirst()) {
            ...
            if (HttpUtil.networkAvailable(context))
                soundUrl=cursor.getString(cursor.getColumnIndex("sound"));
            else {
            ...
            if (HttpUtil.networkAvailable(context)) {
                ...
                show_exa_sentence.setText("当前无缓存且网络不可用");
            }
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.sound:
                if (wordDataList.size() != 0 && HttpUtil.networkAvailable(context)) {
                    ...
                    if (isFromHitDataList && hitDataList.size() != 0) {
                        ...
                        if (wordDataList.get(0).getExampleSentence().equals("null")) {
                            if (HttpUtil.networkAvailable(context))
                                ...
                                show_exa_sentence.setText("该单词例句无缓存且网络不可用");
                                show_exa_sentence.setGravity(Gravity.CENTER);
                            }
                        } else {
            ...
            case R.id.favorites:
                ...
                db.close();
                Log.e("FavDebug", "wordId: " + wordId + " name: " + name);
    ...
    private void prepareNextData() {
        ...
        if (isFromHitDataList && hitDataList.size() != 0) {
            ...
            if (HttpUtil.networkAvailable(context))
                ...
                Toast.makeText(context, "由于网络不可用发音初始化失败", Toast.LENGTH_LONG).show();
    ...
    private class MyAsyncTask extends AsyncTask<String, Integer, String> {
        ...
        protected void onPostExecute(String params) {
            ...
            if (params != null) {
                try {
                    text = WordSenDataSet.parseXmlWithPull(params, true);
